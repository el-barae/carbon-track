...
export default function AdminSection({ 
  ...
  const checkAdminAccess = (): boolean => {
    ...
    if (userAddress) {
      return adminAddresses.some(addr => 
  ...
  const handleMintCredits = async () => {
    if (!hasAdminAccess || !mintRecipient || !mintAmount || !mintProjectId) return
  ...
  const handleVerifyCredit = async (status: "verified" | "rejected") => {
    if (!hasAdminAccess || !verifyHolder) return
  ...
  // Si l'utilisateur est admin, afficher le composant complet
  return (
    <section className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
